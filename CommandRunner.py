...
class CommandError(Exception):
    ...
    def log(self):
        ...
        # Write to log file :
        logger.write("ERROR", {"message":self.message, "raised by":f"file : {os.path.basename(__file__)}\n    instance : {self}\n    called by : {inspect.stack()[1].function}\n    running command : {self.command}"})
    ...
    def __init__(self, command, logger, sudo_required = False, sudo_password = None):
        ...
        if sudo_required:
            if sudo_password is None:
                ...
                # Write to log file :
                logger.write("INFO", {"message":f"sudo password provided manually while running command {self.command}"})
        self.process = None
        self.stop_event = threading.Event()
        self.thread = None
        # Write to log file :
        logger.write("ACTION", {"action":"initialize CommandRunner instance", "invoker":f"file : {os.path.basename(__file__)}\n    instance : {self}\n    called by : {inspect.stack()[1].function}", "output":"0"})
