...
class CommandRunner:
    """
    Run commands inside a non-blocking separate thread and read stdout/stderr streams to print them
    """
        ...
        if sudo_required:
            if sudo_password == None:
                try:
                    ...
                    raise CommandError(f"no sudo password given as argument or defined in keyring", self.command)
                    sys.exit(1)
            else:
    ...
    def _stream_reader(self, stream, stream_name):
        """
        Read lines from the specified stream and print them (with the stream name)
        """
        while not self.stop_event.is_set():
            output = stream.readline()
            if output:
                print(f"{stream_name}: {output.strip()}")
    ...
    def run(self):
        ...
        self.thread.daemon = True  # Make the thread a daemon to ensure proper closing if the parent is killed
        self.thread.start()
